...
def execute(filters=None):
	...
	for inv in invoice_list:
		...
		row = [inv.name, inv.posting_date, inv.supplier,
			inv.bill_no, inv.bill_date, supplier_info.get(inv.supplier), inv.taxes_and_charges]
...
def get_columns(invoice_list):
	...
	columns = [
		_("Invoice") + ":Link/Purchase Invoice:120", _("Posting Date") + ":Date:80", _("Supplier") + ":Link/Supplier:120", 
		_("Bill No") + "::120", _("Bill Date") + ":Date:80", _("TIN #") + "::150", _("Purchase Type") + "::150"
		]
...
def get_invoices(filters):
	...
	return frappe.db.sql("""select name, posting_date, credit_to, supplier, supplier_name, taxes_and_charges,
