...
class eWayBill(Document):
	...
	def get_eway_bill_print(self):
		if self.eway_bill_no:
			if self.valid_upto:
				query = """SELECT name FROM `tabFile` WHERE file_name LIKE '%s.pdf' AND attached_to_doctype = '%s' 
				AND attached_to_name = '%s'"""%(self.eway_bill_no, self.doctype, self.name)
				...
				if not file_list:
					...
					frappe.throw(
						'eWay Bill print already attached for {}'.
							format(frappe.get_desk_link(self.doctype, self.name)))
			else:
				frappe.throw('{} is Not Valid Update Part-B First'.format(frappe.get_desk_link(self.doctype,
																								   self.name)))
		else:
			frappe.throw('eWay Bill No not generated for {}'.format(frappe.get_desk_link(self.doctype,
																								   self.name)))
	...
	def get_eway_bill(self):
		...
		if self.eway_bill_no:
			...
			if self.generated_by == gstin:
				...
				created_by = "others"
			ewb_from_ewb_detail(json_reply, created_by=created_by)
			self.get_eway_bill_print()
		else:
			frappe.throw('No eWay Bill number mentioned for {}'.format(frappe.get_desk_link('eWay Bill', self.name)))
